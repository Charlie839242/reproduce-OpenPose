...
def lightweight_openpose(inputs, num_joints, num_pafs, is_training=True):
    with tf.compat.v1.variable_scope('BackBone', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        net = conv_dw(net, 512, training=is_training)
        net = conv_dw(net, 512, training=is_training)
    with tf.compat.v1.variable_scope('Cpm', reuse=tf.compat.v1.AUTO_REUSE):
    ...
    with tf.compat.v1.variable_scope('InitialStage', reuse=tf.compat.v1.AUTO_REUSE):
    ...
    with tf.compat.v1.variable_scope('RefinementStage', reuse=tf.compat.v1.AUTO_REUSE):
...
if __name__ == '__main__':
    ...
    with tf.compat.v1.Session() as sess:
        with tf.compat.v1.Graph().as_default() as tf_graph:
