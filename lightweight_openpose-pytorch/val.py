...
# This function pad the wdith axis of the sacled_img
# min_dims = [net_input_height_size, max(scaled_img.shape[1], net_input_height_size)]
def pad_width(img, stride, pad_value, min_dims):
    h, w, _ = img.shape                                                 # 得到经过一定处理的scaled_img的长宽
    h = min(min_dims[0], h)
    min_dims[0] = math.ceil(min_dims[0] / float(stride)) * stride       # stride步长设置为8
